...
// Define timespec to be used in timeout:
const struct timespec timeout_ts = {TIMEOUT, 0};
...
/* -------- My Functions -------- */
int sem_timeout(int id, short unsigned int num) {
  struct sembuf op[] = {
    {num, -1, SEM_UNDO}
  };
  // Set semtimedop using timeout_ts defined above and return status:
  int val = semtimedop(id, op, 1, &timeout_ts);
  return val;
}
